...
# ---------------------------------------------------------------------------
# difference_service: content-based MIME resolution + whitelist matching (§7.4.1)
# ---------------------------------------------------------------------------
...
class DispatchMimeResolver(MimeResolver):
    """MIME resolution for *plugin dispatch* (SPECIFICATION.md §4).

    Deliberately more permissive than the inherited ``MimeResolver``, and the
    difference is worth stating because copying the stricter one here was a real
    bug: ``folder_actions`` uses MIME as a **security whitelist**, so it sniffs
    content only, ignores the file name, and treats "unrecognised" as no-match —
    a renamed file must not slip past a filter. Here MIME only decides **which
    diff plugin runs**. Choosing wrongly yields a poor or failed diff, never a
    privilege gain, and the plugin must degrade regardless.

    So this resolver adds the file-name fallback that dispatch needs — without it
    every ``text/plain`` file resolves to ``None`` and is reported "unsupported",
    because content sniffing alone cannot distinguish plain text from arbitrary
    bytes. Events carry no MIME (EVENT_CONTRACT: "derive it from a FileEngine
    stat/content sniff"), so this is the only place the type is decided.

    Order: content signature first (authoritative — a PDF is a PDF whatever it is
    called), then the file name, then ``application/octet-stream``.
    """
    ...
    def resolve(self, file_uid: str) -> str | None:
        ...
        try:
            ...
            try:
                ...
                pass
            sniffed = detect(data or b"", name=name)
            if sniffed:
                result = sniffed
        except Exception:
            result = None
